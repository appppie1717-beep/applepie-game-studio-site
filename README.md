...
최신 세계관을 포함한 정식 문서 16개를 `index, follow`로 제공하고 사이트맵에 넣습니다. 과거 방침 4개도 포함하되 보관본 표시와 당시 본문은 보존합니다. 시크릿과 벨시엔 기업 페이지 3개는 검색을 허용하지만 일반 메뉴에는 노출하지 않습니다. 기업 페이지의 독립된 화면·문구·맞춤 푸터를 보존합니다. `/games`와 slash·`.html`·index 별칭은 정식 주소로 301 이동하며 별도 검색 문서로 만들지 않습니다.
