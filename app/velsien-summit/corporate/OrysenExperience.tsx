...
const networkNodes: NetworkNode[] = [
  { id: "central", name: "중앙 계약 허브", code: "C-00", level: "상층 / 기준점", x: 49, y: 48, state: "stable", signal: "99.98%", detail: "세 기업 간 장기 계약의 중립 검증 노드" },
...
const guaranteeLayers: Array<{ key: LayerKey; code: string; name: string; short: string; score: number; promise: string; tradeoff: string; nodes: string }> = [
  { key: "continuity", code: "L0", name: "생애 연속성", short: "삶의 전환을 끊지 않음", score: 96, promise: "질병·이사·가족 변화가 생겨도 하나의 계약 기록으로 이어집니다.", tradeoff: "서비스를 오래 유지하는 대신, 계약 해지와 데이터 이관에 심사가 필요합니다.", nodes: "24 / 27" },
...
export default function OrysenExperience({ onSwitch = () => undefined }: OrysenExperienceProps) {
  ...
  const nextActionDetail = selectedNode.state === "locked" ? "이동축 권한이 잠겨 있어 새 보장 레이어를 적용하기 전에 권한 심사가 필요합니다." : `${selectedNode.name}에 ${selectedLayer.name}을 연결하면 다음 계약 갱신 때 조건이 함께 검토됩니다.`;
    ...
    <div className={styles.experience} id="top">
      ...
      <div className={styles.consoleLayout}>
        ...
        <main className={styles.consoleMain}>
          <section className={styles.consoleIntro} aria-labelledby="orysen-title"><div className={styles.introCopy}><span className={styles.sectionTag}><i /> 계약 관제 샘플 / 세계관 기준 2187</span><h1 id="orysen-title">불확실성을<br /><em>운영 가능한 상태로.</em></h1><p>오리센은 한 번의 약속보다, 매일 갱신되는 보장 경로를 설계합니다. 벨시엔의 신체·주거·이동 기록을 하나의 생활망으로 연결하세요.</p></div><div className={styles.introMetrics}><Metric label="생활망 안정성" value="99.98%" note="세계관 참고값 · 시뮬레이션" tone="accent" /><Metric label="계약 연속성" value="18.4년" note="운용 모델 예시" /><Metric label="검증 대기" value="04건" note="검토 상태 예시" /></div></section>
          ...
          <section className={styles.competitionSection} aria-label="경쟁사 비교 이동"><div><span className={styles.sectionTag}>벨시엔 / 기업 선택</span><h2>같은 도시를,<br /><em>서로 다른 기준으로.</em></h2><p>오리센은 지속성을 우선합니다. 비렌타와 네릭스의 제안이 궁금하다면 각자의 운영 철학을 직접 비교하세요.</p></div><div className={styles.competitorLinks}><button type="button" onClick={() => onSwitch("virenta")}><span>VIRENTA</span><strong>원하는 삶을 편집하다</strong><b>↗</b></button><button type="button" onClick={() => onSwitch("neryx")}><span>NERYX</span><strong>한계를 시스템으로 넘다</strong><b>↗</b></button></div></section>

          <footer className={styles.consoleFooter}><div><strong>ORYSEN / 오리센</strong><span>내일을 보장하는 생활망 운영사</span></div><div><span>벨시엔 서밋 세계관 기준 · 참고용 시뮬레이션</span><a href="/velsien-summit">벨시엔 홈 ↗</a><button type="button" onClick={openDesk}>계약 데스크 ↗</button></div></footer>
        </main>
      </div>
