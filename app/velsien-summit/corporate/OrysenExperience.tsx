...
export default function OrysenExperience({ onSwitch = () => undefined }: OrysenExperienceProps) {
  ...
  const deskPanelRef = useRef<HTMLElement>(null);
  const deskCloseRef = useRef<HTMLButtonElement>(null);
  ...
  const openDesk = () => {
    deskTriggerRef.current = document.activeElement instanceof HTMLElement ? document.activeElement : null;
    setShowDesk(true);
  };
  ...
  useEffect(() => {
    ...
    deskCloseRef.current?.focus();
    const handleKeyDown = (event: KeyboardEvent) => {
      if (event.key === "Escape") {
        event.preventDefault();
        setShowDesk(false);
      }
      if (event.key !== "Tab" || !deskPanelRef.current) return;
      const controls = Array.from(deskPanelRef.current.querySelectorAll<HTMLElement>('a[href], button:not([disabled]), [tabindex]:not([tabindex="-1"])'));
      if (!controls.length) {
        event.preventDefault();
        deskPanelRef.current.focus();
        return;
      }
      const first = controls[0];
      const last = controls[controls.length - 1];
      if (event.shiftKey && (document.activeElement === first || !deskPanelRef.current.contains(document.activeElement))) {
        event.preventDefault();
        last.focus();
      } else if (!event.shiftKey && (document.activeElement === last || !deskPanelRef.current.contains(document.activeElement))) {
        event.preventDefault();
        first.focus();
      }
    };
  ...
  return (
    <div className={styles.experience} id="top">
      <header className={styles.commandHeader}>
        ...
        <div className={styles.headerTelemetry} aria-label="오리센 시스템 상태"><span><i className={styles.pulse} /> 도시망 연결 예시</span><span>도시 연도 2187</span><span className={styles.headerMode}>운영 모드 / 안정 우선</span></div>
      ...
      <div className={styles.consoleLayout}>
        <aside className={styles.contractRail} aria-label="오리센 계약 탐색">
          ...
          <button type="button" className={styles.railAction} onClick={openDesk}><span>보장 설계 예시 보기</span><b>↗</b></button>
        </aside>

        <main className={styles.consoleMain}>
          <section className={styles.consoleIntro} aria-labelledby="orysen-title"><div className={styles.introCopy}><span className={styles.sectionTag}><i /> 계약 관제 샘플 / 2187</span><h1 id="orysen-title">삶의 연결을<br /><em>끊기지 않게.</em></h1><p>신체 점검, 주거 안전, 도시 이동을 한 계약 기록으로 연결합니다. 변화가 생겨도 필요한 보장이 이어지도록 경로를 갱신합니다.</p></div><div className={styles.introMetrics}><Metric label="생활망 안정성" value="99.98%" note="세계관 참고값 · 시뮬레이션" tone="accent" /><Metric label="계약 경과" value="18.4년" note="한 계약자 운용 예시" /><Metric label="검증 대기" value="04건" note="검토 상태 예시" /></div></section>

          <section className={styles.networkSection} id="network" aria-labelledby="network-title"><div className={styles.panelHeading}><div><span className={styles.sectionTag}>01 / 생활망 상태판</span><h2 id="network-title">도시 전체를 읽고,<br /><em>보장의 빈틈을 찾습니다.</em></h2></div><div className={styles.panelHeadingRight}><span className={styles.liveBadge}><i /> 관제 샘플 / 5개 권역</span><span className={styles.mutedLabel}>참고 지표 · 세계관 기준 시뮬레이션</span></div></div>
          ...
          <section className={styles.termsSection} id="terms" aria-labelledby="terms-title"><div className={styles.termsSignal} aria-hidden="true"><span /><span /><span /><span /><span /></div><div className={styles.termsCopy}><span className={styles.sectionTag}>04 / 오리센의 조건</span><h2 id="terms-title">안전망이 넓어질수록,<br /><em>맡기는 것도 분명해져야 합니다.</em></h2><p>오리센은 시민의 삶을 오래 관리합니다. 그만큼 계약자는 자신이 무엇을 얻고 어떤 통제를 받아들이는지 언제든 읽을 수 있어야 합니다.</p></div><div className={styles.termsColumns}><div><span>얻는 것</span><strong>예측 가능한 내일</strong><p>의료·주거·이동·가족 권한이 하나의 보장 기준으로 연결됩니다.</p></div><div><span>맡기는 것</span><strong>장기 기록과 선택의 일부</strong><p>생활망은 당신의 위험을 계산하기 위해 계속해서 상태를 읽습니다.</p></div><button type="button" onClick={openDesk}>선택한 조건 미리 보기 <b>↗</b></button></div></section>

          <section className={styles.competitionSection} aria-label="경쟁사 비교 이동"><div><span className={styles.sectionTag}>벨시엔 / 기업별 철학</span><h2>같은 도시를,<br /><em>서로 다른 기준으로.</em></h2><p>세 기업은 모두 의료·주거·생산을 포함한 같은 업종을 운영하고 인프라와 군사력도 거의 대등합니다. 오리센은 그중 삶의 지속성을 우선합니다. 비렌타와 네릭스가 무엇을 먼저 지키는지도 확인해 보세요.</p></div><div className={styles.competitorLinks}><button type="button" onClick={() => onSwitch("virenta")}><span>VIRENTA</span><strong>원하는 삶을 편집하다</strong><b>↗</b></button><button type="button" onClick={() => onSwitch("neryx")}><span>NERYX</span><strong>한계를 시스템으로 넘다</strong><b>↗</b></button></div></section>
      ...
      {showDesk && <div className={styles.deskOverlay} role="presentation"><button type="button" className={styles.deskBackdrop} onClick={() => setShowDesk(false)} aria-label="계약 데스크 닫기" tabIndex={-1} /><aside ref={deskPanelRef} className={styles.deskPanel} role="dialog" aria-modal="true" aria-labelledby="desk-title" tabIndex={-1}><div className={styles.deskPanelTop}><span className={styles.sectionTag}><i /> 오리센 전용 계약 데스크</span><button ref={deskCloseRef} type="button" className={styles.deskClose} onClick={() => setShowDesk(false)} aria-label="계약 데스크 닫기">×</button></div><div className={styles.deskPanelCode}>계약 검토 예시 / {selectedNode.code} · {selectedLayer.code}</div><h2 id="desk-title">보장 경로를 확인하고,<br /><em>다음 조치를 살펴봅니다.</em></h2><p className={styles.deskLead}>현재 선택한 노드와 보장 레이어를 기준으로 계약자가 확인할 내용을 정리했습니다.</p><dl className={styles.deskFacts}><div><dt>선택 노드</dt><dd>{selectedNode.name}<small>{selectedNode.level}</small></dd></div><div><dt>보장 레이어</dt><dd>{selectedLayer.name}<small>신뢰 기준점 {selectedLayer.score}</small></dd></div><div><dt>현재 상태</dt><dd className={styles[`text${selectedNode.state[0].toUpperCase()}${selectedNode.state.slice(1)}`]}>{nodeStatus}<small>{selectedNode.signal} · 참고값</small></dd></div></dl><div className={styles.deskNext}><span>권고 다음 조치</span><strong>{nextAction}</strong><p>{nextActionDetail}</p></div><div className={styles.deskActions}><button type="button" className={styles.deskPrimary} onClick={() => setShowDesk(false)}>미리 보기 닫기 <b>×</b></button></div><small className={styles.deskDisclaimer}>벨시엔 서밋 세계관 기준 · 실제 계약이 아닌 운영 화면 시뮬레이션</small></aside></div>}
    </div>
  );
}
